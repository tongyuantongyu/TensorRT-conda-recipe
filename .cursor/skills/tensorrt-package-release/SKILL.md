...
description: Update TensorRT recipe versions, refresh source hashes from rattler-build caches, rebuild Windows and WSL/Linux package matrices, verify package outputs, and publish successful artifacts to the local private channel. Use when bumping TensorRT, refreshing source hashes, rerendering the build graph, checking repodata, or publishing repaired packages.
---
...
- Update source URL templates only if the upstream URL scheme changed.
- If the URL scheme appears unchanged, defer `sha256` updates and let `rattler-build` populate `src_cache` first.
- Update `recipe/variants.yaml` if the supported CUDA matrix changed.

2. Refresh checksums through the build cache.
- For unchanged URL schemes, intentionally keep old `sha256` values and run narrow checksum-populating builds.
- Use `--up-to libnvinfer10 --test skip` plus one `cuda_version` override so each command downloads one source archive and stops at the expected checksum failure.
- Copy the reported `actual:` SHA-256 into `recipe/recipe.yaml`.
- Once the checksum-failure behavior is known, running different archive downloads in parallel is safe; the risk is only bandwidth/CDN throttling, not cache corruption, because each archive has a distinct cache filename.
- Windows examples:

```powershell
mamba run --live-stream -n base rattler-build build --package-format conda:max --no-include-recipe --up-to libnvinfer10 --test skip -r recipe.yaml -m variants.yaml -m variants.win.yaml --variant cuda_version=12.9 --variant python=3.13 -c conda-forge
mamba run --live-stream -n base rattler-build build --package-format conda:max --no-include-recipe --up-to libnvinfer10 --test skip -r recipe.yaml -m variants.yaml -m variants.win.yaml --variant cuda_version=13.2 --variant python=3.13 -c conda-forge
```

- Linux examples from the Windows host:

```powershell
wsl.exe bash -ic "cd /mnt/c/Users/TYTY/Libraries/conda-packages/tensorrt/recipe/ && mamba run -n base rattler-build build --package-format conda:max --no-include-recipe --up-to libnvinfer10 --test skip -r recipe.yaml -m variants.yaml -m variants.linux.yaml --variant cuda_version=12.9 --variant python=3.13 -c conda-forge --output-dir ~/project/conda-packages"
wsl.exe bash -ic "cd /mnt/c/Users/TYTY/Libraries/conda-packages/tensorrt/recipe/ && mamba run -n base rattler-build build --package-format conda:max --no-include-recipe --up-to libnvinfer10 --test skip -r recipe.yaml -m variants.yaml -m variants.linux.yaml --variant cuda_version=13.2 --variant python=3.13 -c conda-forge --output-dir ~/project/conda-packages"
```

3. Start with the happy-path assumption.
- It is fine to render or build once assuming the upstream archive layout is unchanged after hashes are refreshed.
- If a failure shows missing files, wrong archive names, or copy/test path errors, inspect the relevant `src_cache` under `recipe/output` for Windows or `~/project/conda-packages` for Linux, then patch the recipe to match the extracted upstream layout.
...
4. Sanity-check the rendered graph when the recipe changed materially.
- Windows render:
...
```powershell
wsl.exe bash -ic "cd /mnt/c/Users/TYTY/Libraries/conda-packages/tensorrt/recipe/ && mamba run -n base rattler-build build -r recipe.yaml -m variants.yaml -m variants.linux.yaml -c conda-forge --output-dir ~/project/conda-packages --render-only"
```

- Use the rendered job list to confirm whether only the Python-bearing outputs vary by Python.

5. Keep the staging area clean.
...
- Remove old staged `.conda` files before a full rebuild if they are already published to `D:/PyEnv/channels/private`; otherwise move them aside.
- Always inspect output roots via the terminal, not file-search tools, because they may be ignored or outside the repo.
- Before a full rebuild, clean the target platform output directory. Also clear stale `recipe/output/linux-64` if Linux builds now use the WSL output root, so Windows-side indexing does not carry old Linux packages.
...
Get-ChildItem -LiteralPath "C:\Users\TYTY\Libraries\conda-packages\tensorrt/recipe/output/linux-64" -Filter *.conda -ErrorAction SilentlyContinue | Remove-Item -Force
```

- Linux/WSL cleanup example from the Windows host:

```powershell
wsl.exe bash -lc 'ls -1 "$HOME"/project/conda-packages/linux-64/*.conda 2>/dev/null | wc -l'
wsl.exe bash -lc 'rm -f "$HOME"/project/conda-packages/linux-64/*.conda'
```
...
- If the user asked to finish the release, poll in long slices until completion; otherwise, after startup looks healthy, ask the user to report when the build ends or send the failure snippet.
...
- Count WSL/Linux outputs with a simple WSL command from PowerShell:
...
- Reindex both staging roots before auditing:
...
9. Publish successful artifacts to the local channel.
...
```powershell
wsl.exe bash -lc 'cp -f "$HOME"/project/conda-packages/linux-64/*.conda /mnt/d/PyEnv/channels/private/linux-64/'
```

- Then rebuild the private channel index:

```powershell
mamba run --live-stream -n base python -m conda_index "D:/PyEnv/channels/private" --zst
```

- After publishing, count the published target-version packages and rerun the CUDA dependency audit against `D:/PyEnv/channels/private/*/repodata.json`.

10. Reindex the staging area after moving packages out.
